## NPM Install
npm_install_all:
	cd sp_mocks/wgp/frontend && npm install && cd ../backend && npm install 
...
build_sp_mocks_frontend_image:
	cd sp_mocks/wgp/frontend && docker build --tag wgp_frontend --file Dockerfile .

build_sp_mocks_backend_image:
	cd sp_mocks/wgp/backend && docker build --tag wgp_backend --file Dockerfile .

# To build all images at once
build_images:
	make build_server_image && make build_wgp_frontend_image && make build_wgp_backend_image

run_layer8_server_image:
	docker run -p 5001:5001 -t layer8-server

run_wgp_frontend_image:
	docker run -p 8080:8080 -t wgp_frontend

run_wgp_backend_image:
	docker run -p 8000:8000 -t wgp_backend

push_layer8_server_image:
	aws lightsail push-container-image --region ca-central-1 --service-name aws-container-service-t1 --label layer8-server-version-7 --image layer8-server:latest

push_wgp_frontend_image:
	aws lightsail push-container-image --region ca-central-1 --service-name container-service-2 --label frontendversion4 --image wgp_frontend:latest

push_wgp_backend_image:
	aws lightsail push-container-image --region ca-central-1 --service-name container-service-3 --label backendversion5 --image wgp_backend:latest

push_images:
	make push_layer8_server_image && make push_wgp_frontend_image && make push_wgp_backend_image
