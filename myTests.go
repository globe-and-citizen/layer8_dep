...
func main() {
	privJWK, _, _ := utils.GenerateKeyPair(utils.ECDH)
	privateKeyasInterface, _ := privJWK.ExportKeyAsGoType()
	var privKeyGo *ecdh.PrivateKey
	if pk, ok := privateKeyasInterface.(*ecdh.PrivateKey); !ok {
		panic("god damn it...")
	} else {
		privKeyGo = pk
	}

	fmt.Printf("%T", privKeyGo)
